...
#### For positive tests results: 
  * If tested positive for RSV more than once within 14 days (difference of testing date <= 14 days), keep only one most recent RSV+ record.
  
##### Table `r tab.counter` Those with more than one positive tests within 14 days:
```{r echo = T}
tab.counter <- tab.counter + 1

# those with 2 positive tests
...
# only keep the most recent RSV+ record
testID.temp <- (df %>% filter(StudyID %in% id.temp) %>% filter(positive_rsv ==1 ) %>% 
  group_by(StudyID) %>% filter(collection_date == min(collection_date)))$testID
df <- df %>% filter(testID %in% testID.temp == F)
...
testID.toremove2 <- (a %>% group_by(StudyID) %>% mutate(n_tests_temp = n()) %>% filter(n_tests_temp == 2) %>%
  ...
  dplyr::select(testID, positive_rsv, encounter_type, n) %>% 
  filter(n == 2 & encounter_type == "outpatient"))$testID
# 2.2) if the two testing records have same encounter_type,  keep the latest negative test 
testID.toremove3 <- (a %>% group_by(StudyID) %>% mutate(n_tests_temp = n()) %>% filter(n_tests_temp == 2) %>%
  ...
  filter(n == 1) %>%
  group_by(StudyID) %>%
  filter(collection_date == min(collection_date)))$testID
...
testID.toremove <- c(
                     ...
                     "130752807_1",
                     "130780764_2",
                     # repeated earlier RSV- record in outpatient
                     "130632441_2",
                     "130657344_1",
                     "130702757_1",
                     "130715070_1",
...
if(save_recoded_dataset){
  write.csv(df.ssa, "../../data/clean_data/df.mab.ssa.csv", row.names = F)
}
