...
df %>% filter(is.na(symp_URT_distress) & is.na(symp_LRT_distress) & is.na(symp_fever) & is.na(symp_cough) & is.na(symp_wheezing) & is.na(symp_breathing) & is.na(symp_bronchiolitis) & is.na(symp_sepsis & is.na(symp_apnea)) &
                     !str_detect(`Primary Problem`, "respiratory|RSV|syncytial")  & 
                     !str_detect(`ED Dx`, "respiratory|RSV|syncytial")  & 
                     !str_detect(`Admit Dx`, "respiratory|RSV|syncytial") & 
                     !str_detect(`Hospital Problem List`, "respiratory|RSV|syncytial")
                   ) %>% 
...
#### Generated new binary variables rsv_mab, to have values of 
  * 50mg
  * 100mg 
...
```{r}
df <- df %>% 
  mutate(rsv_mab = case_when(`Nirsevimab info` == "RSV, MAB, NIRSEVIMAB-ALIP, 50 MG/0.5 ML, FOR PATIENTS < 5 KG [306]" ~  "50mg",
                             `Nirsevimab info` == "RSV, MAB, NIRSEVIMAB-ALIP, 100 MG/ML, FOR PATIENTS >= 5 KG [307]"  ~ "100mg",
