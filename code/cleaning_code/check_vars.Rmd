...
df %>% filter(icu_admitted == "yes") %>% filter(icu_los_days > 30)%>% 
  select(StudyID, `Collection Date`, date_icu_admit, icu_los_days) %>%
  reactable(style = list(color = "black", `font-family` = "Georgia", `font-size` = "15px"))
```
...
```{r}
df <- df %>% select(-c(risk_factor_earlyterm, risk_factor_fullterm, risk_factor_preterm, risk_factor_postterm))
```
...
df <- df %>%
  mutate(risk_factor_atleastone = case_when(is.na(risk_factor_anemia) & 
