...
#### The team helped checked above records and classified them suspected respiratory infections or not, remove those that are not suspected respiratory infections. Deleted 212 records.
```{r}
...
for(i in 1:nrow(df.not.suspect)){
  id <- df.check.suspect[i,]$StudyID
  cd <- df.check.suspect[i,]$collection_date
...
a <- df.multi.tests %>% dplyr::select(StudyID, `mAb date`,
...
temp <- df %>% 
  ...
  dplyr::select(StudyID, n_controls_to_sample_from) %>% unique()
...
df %>% filter(StudyID %in% id_multi_cases) %>% 
  dplyr::select(StudyID, collection_date, tested_viruses, positive_viruses, case_control) %>%
  reactable(style = list(color = "black", `font-family` = "Georgia", `font-size` = "15px"), 
            groupBy = "StudyID", defaultExpanded = T) 
```
