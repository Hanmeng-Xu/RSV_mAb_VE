...
### Figure `r fig.counter`. Display age at testing and remove records above 1 yo
  * Remove records with (collection date - DOB) > 12 months. (TBC? age limit?)
...
### Figure `r fig.counter`. Sex & Age at testing (after removing age at testing > 1 yo)
```{r}
fig.counter <- fig.counter + 1
...
#### what tests are included in "RESPIRATORY VIRUS PCR PANEL     (BH GH LMW Q YH) [LAB3444]"? (TBC)
```{r}
...
There might be some uncertainty about this variable so we might not trust this 100%. Here calculated time difference (in days) between Admit Date and sample collection. (this variable might not be that useful in the actual analyses.)
```{r}
tab.counter <- tab.counter + 1
...
Length of Stay (in days) of hospitalization recorded in the main dataset. **Can this be a measure of severity?**
```{r}
tab.counter <- tab.counter + 1
...
**Can I recode the Hosp LoS of these to 1 (TBC)?**
```{r}
...
There are 35 missings for original "encounter type" variable. Most seem to be from LABs or DRAW STATION except one record from "NE PEDS OLD SAYBROOK", **can I classify them as "Specimen" ? (TBC)**
```{r}
tab.counter <- tab.counter + 1
...
#### Generated new variables:
  ...
  + date_icu_admit_related: ICU admission that can be linked with the testing result (**within a week of sample collection date? TBC**)
...
### Figure `r fig.counter`. Birth weight
  ...
  * what is the unit of original variable Birth Wgt? Haven't generated new variables (TBC?).
```{r}
fig.counter <- fig.counter + 1
...
There are many conflicting answers (TBC?). Here are the unique answers for Race variable:
```{r}
tab.counter <- tab.counter + 1
...
### Below are text patterns to detect risk factors (i.e. underlying conditions) and respiratory symptoms from the following variables (TBC):
...
This table is a subset of table 20 above. (all records here are from outpatient)
```{r}
tab.counter <- tab.counter + 1
...
#### Generate new variable max_oxygen_flowrate (unit: L/min), which threshold to use to indicate severe outcome? TBC
```{r}
fig.counter <- fig.counter + 1
