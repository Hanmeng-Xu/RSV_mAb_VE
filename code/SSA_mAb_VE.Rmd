...
### Sensitivity analysis: after changing defintion for vaccination to those immunized >= 7 days before sample collection. 
...
```{r}
# read in dataset
df <- read.csv("../data/clean_data/df.mab.ssa.csv") %>% 
...
plt.collection.date <- 
  ...
  geom_text(aes(x = as.Date("2023-10-5"), y = 150, label = "Nirsevimab administration starts"), angle = 90, size = 3.5, fontface = "plain")
  

plt.collection.date

if(savenewplot){
  ggsave(plot = plt.collection.date, filename = "../figs_tabs/figures/SSA.collection.date.pdf", height = 4, width = 8)
}
```
...
if(savenewplot){
  table1_infection %>%
    as_gt() %>%
    gt::gtsave(filename = "../figs_tabs/tables/SSA.table1_characteristics.docx")
}
```
...
if(savenewplot){
  table1_outcome %>%
    as_gt() %>%
    gt::gtsave(filename = "../figs_tabs/tables/SSA.table2_clinical_outcomes.docx")
}
```
...
if(savenewplot){
  ggsave(plot = plt.wane.2m, filename = "../figs_tabs/figures/SSA.wane.2m.pdf", height = 4, width = 8)
}
```
...
if(savenewplot){
  ggsave(plot = plt.ve.dose.addanydose, filename = "../figs_tabs/figures/SSA.ve.dose.addanydose.pdf", height = 10, width = 12)
}
