...
model_string_waning_2monthsinterval <- "
...
model{
  ...
  for(j in 1:N_records){
    ...
    logit(pi[j]) <- 
      int + 
      vax[j] * vax_time_group1[j] * beta1 +  # vax_time_group1[j] = 1 if this individual took mAb 0-2 months ago, otherwise vax_time_group1[j] = 0
      vax[j] * vax_time_group2[j] * beta2 +  # vax_time_group2[j] = 1 if this individual took mAb 2-4 months ago, otherwise vax_time_group2[j] = 0
      vax[j] * vax_time_group3[j] * beta3 +  # vax_time_group3[j] = 1 if this individual took mAb >4 months ago, otherwise vax_time_group3[j] = 0
      cf_age_1[j] * beta_cf_age_1 + # dummy variables for categorical confounders
      cf_age_2[j] * beta_cf_age_2 +
      cf_age_3[j] * beta_cf_age_3 +
      cf_age_4[j] * beta_cf_age_4 +
  ...
  #beta_cf_age ~ dnorm(0, 1e-04)
  #beta_cf_month_tested ~ dnorm(0, 1e-04)
...
model{
  ...
  for(j in 1:N_records){
    ...
    logit(pi[j]) <- 
      int + 
      vax[j] * vax_time_group1[j] * beta1 +  # vax_time_group1[j] = 1 if this individual took mAb 0-1 months ago, otherwise vax_time_group1[j] = 0
      vax[j] * vax_time_group2[j] * beta2 +  # vax_time_group2[j] = 1 if this individual took mAb 1-2 months ago, otherwise vax_time_group2[j] = 0
      vax[j] * vax_time_group3[j] * beta3 +  # vax_time_group3[j] = 1 if this individual took mAb 2-3 months ago, otherwise vax_time_group3[j] = 0
      vax[j] * vax_time_group4[j] * beta4 +  # vax_time_group4[j] = 1 if this individual took mAb 3-4 months ago, otherwise vax_time_group4[j] = 0
      vax[j] * vax_time_group5[j] * beta5 +  # vax_time_group5[j] = 1 if this individual took mAb 4+ months ago, otherwise vax_time_group5[j] = 0
      cf_age_1[j] * beta_cf_age_1 + # dummy variables for categorical confounders
      cf_age_2[j] * beta_cf_age_2 +
      cf_age_3[j] * beta_cf_age_3 +
      cf_age_4[j] * beta_cf_age_4 +
...
# change the prior stucture of increment 
model_string_waning_1monthinterval_tau <- "
  # classify time since vax to testing into three groups: 0-1,1-2,2-3,3-4,4+ months
...
# change the prior stucture of increment 
model_string_waning_1monthinterval_tau_allcf <- "
  # classify time since vax to testing into three groups: 0-1,1-2,2-3,3-4,4+ months
